...
const AdminNGOs = () => {
    ...
    const handleEdit = (user) => {
        setEditingNGO(user.username);
        setEditedData({ contact: user.contact || '', role: user.role || 'ngo' });
    };

    const handleSave = async (username) => {
        toast.loading("Updating ngo...", { id: "update" });
        try {
            ...
            if (res.ok) {
                toast.success("NGO updated", { id: "update" });
                setEditingNGO(null);
                fetchNgos();
            } else {
            ...
            toast.error("Error updating ngo", { id: "update" });
        }
    };
    ...
    const handleDelete = async (username) => {
        ...
        toast.loading("Deleting ngo...", { id: "delete" });
        try {
            const res = await fetch(`${API}?username=${username}`, { method: "DELETE" });
            const result = await res.json();

            if (res.ok) {
                toast.success("NGO deleted", { id: "delete" });
                fetchNgos();
            } else {
                toast.error(result.error || "Delete failed", { id: "delete" });
            }
        } catch (error) {
            toast.error("Error deleting ngo", { id: "delete" });
        }
    };
